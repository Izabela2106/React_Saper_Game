...
col=parseInt(col);
        ...
        newArray.map(cell=>{
            freeCells.forEach(freecell=>{
                 if(freecell === cell){
                    ...
                    let placed=false;
                    while(!placed){
                        let randomNumber=Math.floor(Math.random()*state.height*state.width);
                        if(!cellsArray[randomNumber].bomb && !freeCells.includes(cellsArray[randomNumber])){
    ...
    newArray=cellsArray.map(cell=>{
        if(row ==cell.row && col==cell.column && !cell.flag && isGameActive && cell.state===1){
            if(cell.bomb){
                isGameActive=false;
    ...
    if(value===0){
            ...
            cellsToReveal.forEach(item=>{
                if(item.r>=0 && item.c >=0 && item.r <state.width && item.c<state.height) {
                    ...
                    let returnObject=revealCell(item.r,item.c,newArray,isGameActive,fieldCellsToReveal);
...
return {cellsArray:cellsArray,toPlace:toPlace,isGameActive:isGameActive,cellsToReveal:cellsToReveal}
        ...
        return {cellsArray:returnObject.newArray,toPlace:toPlace,isGameActive:returnObject.isGameActive,cellsToReveal:returnObject.fieldCellsToReveal}
        ...
        array.map(cell=>{
            let cellsToCount=[
                ...
                cellsToCount.forEach(item=>{
                    if(item.r>=0 && item.c >=0 && item.r <state.width && item.c< state.height){
                    let cell2=array.filter(ele=>(ele.row===item.r && ele.column===item.c))[0];
                    if(cell2.bomb){
                        value++;
                    }
                    }
    ...
    if(action.type==='HANDLE_DIFFICULTY_BUTTON'){
        ...
        return{...state,isModalActive:false,cellSize:level.cellSize, height:level.height,width:level.width,mines:level.mines,flagsToPlace:level.mines,startCellsToReveal:level.height*level.width-level.mines, cellsToReveal:level.height*level.width-level.mines,gameActive:false}
    }
...
let col=action.payload.target.getAttribute('column')
let row=action.payload.target.getAttribute('row')
let cellsArray=[...state.fieldArray];
let toPlace=state.flagsToPlace;
let isGameActive=state.gameActive;
let cellsToReveal=state.cellsToReveal;
let isModal=state.isModalActive;

let returnObject=handleClick(row,col,action.payload.button,toPlace,isGameActive,cellsArray,cellsToReveal);
...
return {...state,fieldArray:cellsArray,flagsToPlace:toPlace,gameActive:isGameActive,cellsToReveal:cellsToReveal,isModalActive:isModal};
